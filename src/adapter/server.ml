...
let handle_message hdl frontend_io subprocess callback config msg =
  let _ic, oc = frontend_io in
  match%lwt Handler.handle_exn hdl subprocess callback config msg with
  | Ok js ->
...
let rec main_handler ~subprocess hdl (config:Dapper.Dap_config.t) content_length flow ic oc =
  ...
  | Some count ->
      ...
      handle_message hdl frontend_io subprocess callback config msg >>= fun _ ->
      let content_length = None in
      main_handler ~subprocess hdl config content_length flow ic oc
  | None -> (
...
and subprocess_start hdl config content_length flow ic oc process_full =
  ...
  Logs_lwt.info (fun m -> m "[DAP] starting subprocess") >>= fun _ ->
  main_handler ~subprocess:(Some process_full) hdl config content_length flow ic oc
...
let svc ~listen_address ~port =
  ...
  Lwt_main.run (
    ...
    Conduit.serve ~on_exn ~ctx ~mode (main_handler ~subprocess:None hdl config content_length)
    >|= fun _ ->
    `Ok ()
  )
