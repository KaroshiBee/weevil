...
type t = | Cancel | RunInTerminal | Initialize | ConfigurationDone | Launch | Attach | Restart | Disconnect | Terminate | BreakpointLocations | SetBreakpoints | SetFunctionBreakpoints | SetExceptionBreakpoints | DataBreakpointInfo | SetDataBreakpoints | SetInstructionBreakpoints | Continue | Next | StepIn | StepOut | StepBack | ReverseContinue | RestartFrame | Goto | Pause | StackTrace | Scopes | Variables | SetVariable | Source | Threads | TerminateThreads | Modules | LoadedSources | Evaluate | SetExpression | StepInTargets | GotoTargets | Completions | ExceptionInfo | ReadMemory | WriteMemory | Disassemble
let enc = 
let open Data_encoding in
conv
