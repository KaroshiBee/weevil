...
module Flow : sig
  ...
  (* NOTE using ErrorResponse_body here because dont know the sequence number yet *)
  type ret = (Dap_message.response * Dap_message.event list, Dap_message.ErrorResponse_body.t) Result.t
  val request_response : config:config -> Dap_message.request -> ret
end = struct
  ...
  let request_response ~config = function
    | CancelRequest req ->
      let resp =
