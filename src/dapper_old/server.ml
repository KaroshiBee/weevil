...
module DRq = Dap_request
module DRs = Dap_response
module DEv = Dap_event
module Db = Dap_base
...
let handle_msg :
  ...
  | Variables req -> (
      ...
      let variables = [
        ...
        stack_val |> List.mapi (fun i sv -> Db.Variable_.{name=Printf.sprintf "%d:" i; value=String.trim sv; variablesReference=0})
      ] |> List.concat
      in
