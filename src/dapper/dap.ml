...
let rec content_length_message_handler
  ...
  | None -> (
      Logs_lwt.info (fun m -> m "waiting for content-length messages")
      >>= fun _ ->
      Lwt_io.read_line_opt ic >>= function
      | Some msg ->
          Logs_lwt.info (fun m -> m "got messsage '%s'" msg) >>= fun _ ->
  ...
  | Some count ->
      Logs_lwt.info (fun m ->
          m "got content-length message with length %d" count)
      >>= fun _ ->
      (* \r\n throw away *)
      ...
      Logs_lwt.info (fun m ->
          m "got content-length message with message '%s'" msg)
      >>= fun _ ->
      handle_message msg ic oc >>= fun _ ->
      let content_length = None in
      content_length_message_handler ~handle_message ~content_length ic oc
