...
let configure_contracts ctxt script balance ~src_opt ~pay_opt ~self_opt =
  ...
  let (source, payer) : Ctxt.Contract.t * Ctxt.public_key_hash =
    ...
    | (None, None) ->
      ...
      (self, Tezos_crypto.Signature.Public_key_hash.zero)
...
let trace_code
  ...
  let now = match now with None -> Prot.Script_timestamp.now ctxt | Some t -> t in
  let*! () = Logs_lwt.debug (fun m -> m "getting level") in
  let level =
    match level with
    | None ->
      (Ctxt.Level.current ctxt).level |> Ctxt.Raw_level.to_int32 |> Prot.Script_int.of_int32
      |> Prot.Script_int.abs
    | Some z -> z
  in
  ...
  let step_constants =
    ...
    let source = Ctxt.Destination.Contract source in
  ...
  let ops = Prot.Apply_internal_results.packed_internal_operations operations in
