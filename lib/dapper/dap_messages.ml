...
urlLabel: (string option [@gen Gen.gen_utf8_str_opt] ); } [@@deriving qcheck, eq]
...
val allThreadsContinued : t -> (bool option  )
 ...
 type t = { threadId: (int [@gen Gen.gen_int31] );
allThreadsContinued: (bool option  ); } [@@deriving qcheck, eq]
...
type t 
 ...
 type t = { exitCode: (int [@gen Gen.gen_int31] ); } [@@deriving qcheck, eq]
...
type t 
 ...
 type t = { restart: (Data_encoding.json option [@gen Gen.gen_json_opt] [@equal Eq.equal_json_opt]); } [@@deriving qcheck, eq]
...
(* dont bother with a sig for enums, the inferred one is fine *)
 ...
 type t = Started | Exited | Other of string [@@deriving qcheck, eq]

 let enc = 
 let open Data_encoding in 
 conv_with_guard 
 (function Started -> "started" | Exited -> "exited" | Other s -> s)
 (function "started" -> Ok Started | "exited" -> Ok Exited | _ as s -> Ok (Other s))
 string
...
threadId: (int [@gen Gen.gen_int31] ); } [@@deriving qcheck, eq]
...
(* dont bother with a sig for enums, the inferred one is fine *)
 ...
 type t = Console | Important | Stdout | Stderr | Telemetry | Other of string [@@deriving qcheck, eq]

 let enc = 
 let open Data_encoding in 
 conv_with_guard 
 (function Console -> "console" | Important -> "important" | Stdout -> "stdout" | Stderr -> "stderr" | Telemetry -> "telemetry" | Other s -> s)
 (function "console" -> Ok Console | "important" -> Ok Important | "stdout" -> Ok Stdout | "stderr" -> Ok Stderr | "telemetry" -> Ok Telemetry | _ as s -> Ok (Other s))
 string
...
(* dont bother with a sig for enums, the inferred one is fine *)
 ...
 type t = Start | StartCollapsed | End [@@deriving qcheck, eq]

 let enc = 
 let open Data_encoding in 
 conv_with_guard 
 (function Start -> "start" | StartCollapsed -> "startCollapsed" | End -> "end")
 (function "start" -> Ok Start | "startCollapsed" -> Ok StartCollapsed | "end" -> Ok End | _ -> Error "OutputEvent_body_group")
 string
...
(* dont bother with a sig for enums, the inferred one is fine *)
 ...
 type t = Normal | Emphasize | Deemphasize [@@deriving qcheck, eq]

 let enc = 
 let open Data_encoding in 
 conv_with_guard 
 (function Normal -> "normal" | Emphasize -> "emphasize" | Deemphasize -> "deemphasize")
 (function "normal" -> Ok Normal | "emphasize" -> Ok Emphasize | "deemphasize" -> Ok Deemphasize | _ -> Error "Source_presentationHint")
 string
...
(* dont bother with a sig for enums, the inferred one is fine *)
 ...
 type t = MD5 | SHA1 | SHA256 | Timestamp [@@deriving qcheck, eq]

 let enc = 
 let open Data_encoding in 
 conv_with_guard 
 (function MD5 -> "MD5" | SHA1 -> "SHA1" | SHA256 -> "SHA256" | Timestamp -> "timestamp")
 (function "MD5" -> Ok MD5 | "SHA1" -> Ok SHA1 | "SHA256" -> Ok SHA256 | "timestamp" -> Ok Timestamp | _ -> Error "ChecksumAlgorithm")
 string
...
checksum: (string [@gen Gen.gen_utf8_str] ); } [@@deriving qcheck, eq]
...
val checksums : t -> (Checksum.t list option  )
 ...
 type t = { name: (string option [@gen Gen.gen_utf8_str_opt] );
...
adapterData: (Data_encoding.json option [@gen Gen.gen_json_opt] [@equal Eq.equal_json_opt]);
checksums: (Checksum.t list option  ); } [@@deriving eq]
...
(option @@ list Checksum.gen)
              in
              map (fun ( name, path, sourceReference, presentationHint, origin, sources, adapterData, checksums ) -> { name; path; sourceReference; presentationHint; origin; sources; adapterData; checksums }) gg
          in
          match n with
          | 0 -> _gen_t basecase
          | n ->
            frequency
              [
                (1, _gen_t basecase);
                (1, let t = map (fun {sources; _} -> sources) @@ self (n / 2) in _gen_t t);
              ]
        )) 
...
data: (Data_encoding.json option [@gen Gen.gen_json_opt] [@equal Eq.equal_json_opt]); } [@@deriving qcheck, eq]
...
(* dont bother with a sig for enums, the inferred one is fine *)
 ...
 type t = Changed | New | Removed | Other of string [@@deriving qcheck, eq]

 let enc = 
 let open Data_encoding in 
 conv_with_guard 
 (function Changed -> "changed" | New -> "new" | Removed -> "removed" | Other s -> s)
 (function "changed" -> Ok Changed | "new" -> Ok New | "removed" -> Ok Removed | _ as s -> Ok (Other s))
 string
...
val offset : t -> (int option  )
 ...
 type t = { id: (int option [@gen Gen.gen_int31_opt] );
verified: (bool  );
...
offset: (int option [@gen Gen.gen_int31_opt] ); } [@@deriving qcheck, eq]
...
breakpoint: (Breakpoint.t  ); } [@@deriving qcheck, eq]
...
(* dont bother with a sig for enums, the inferred one is fine *)
 ...
 type t = New | Changed | Removed [@@deriving qcheck, eq]

 let enc = 
 let open Data_encoding in 
 conv_with_guard 
 (function New -> "new" | Changed -> "changed" | Removed -> "removed")
 (function "new" -> Ok New | "changed" -> Ok Changed | "removed" -> Ok Removed | _ -> Error "ModuleEvent_body_reason")
 string
...
addressRange: (string option [@gen Gen.gen_utf8_str_opt] ); } [@@deriving qcheck, eq]
...
module_: (Module_.t  ); } [@@deriving qcheck, eq]
...
(* dont bother with a sig for enums, the inferred one is fine *)
 ...
 conv_with_guard 
 (function New -> "new" | Changed -> "changed" | Removed -> "removed")
 (function "new" -> Ok New | "changed" -> Ok Changed | "removed" -> Ok Removed | _ -> Error "LoadedSourceEvent_body_reason")
 string
...
source: (Source.t  ); } [@@deriving qcheck, eq]
...
(* dont bother with a sig for enums, the inferred one is fine *)
 ...
 type t = Launch | Attach | AttachForSuspendedLaunch [@@deriving qcheck, eq]

 let enc = 
 let open Data_encoding in 
 conv_with_guard 
 (function Launch -> "launch" | Attach -> "attach" | AttachForSuspendedLaunch -> "attachForSuspendedLaunch")
 (function "launch" -> Ok Launch | "attach" -> Ok Attach | "attachForSuspendedLaunch" -> Ok AttachForSuspendedLaunch | _ -> Error "ProcessEvent_body_startMethod")
 string
...
val pointerSize : t -> (int option  )
 ...
 type t = { name: (string [@gen Gen.gen_utf8_str] );
...
pointerSize: (int option [@gen Gen.gen_int31_opt] ); } [@@deriving qcheck, eq]
...
val conditionDescription : t -> (string option  )
 ...
 type t = { filter: (string [@gen Gen.gen_utf8_str] );
label: (string [@gen Gen.gen_utf8_str] );
description: (string option [@gen Gen.gen_utf8_str_opt] );
default: (bool option  );
supportsCondition: (bool option  );
conditionDescription: (string option [@gen Gen.gen_utf8_str_opt] ); } [@@deriving qcheck, eq]
...
(* dont bother with a sig for enums, the inferred one is fine *)
 ...
 type t = String | Number | Boolean | UnixTimestampUTC [@@deriving qcheck, eq]

 let enc = 
 let open Data_encoding in 
 conv_with_guard 
 (function String -> "string" | Number -> "number" | Boolean -> "boolean" | UnixTimestampUTC -> "unixTimestampUTC")
 (function "string" -> Ok String | "number" -> Ok Number | "boolean" -> Ok Boolean | "unixTimestampUTC" -> Ok UnixTimestampUTC | _ -> Error "ColumnDescriptor_type_")
 string
...
val width : t -> (int option  )
 ...
 type t = { attributeName: (string [@gen Gen.gen_utf8_str] );
label: (string [@gen Gen.gen_utf8_str] );
format: (string option [@gen Gen.gen_utf8_str_opt] );
type_: (ColumnDescriptor_type_.t option  );
width: (int option [@gen Gen.gen_int31_opt] ); } [@@deriving qcheck, eq]
...
supportsGotoTargetsRequest: (bool option  ); } [@@deriving qcheck, eq]
...
supportsExceptionInfoRequest: (bool option  ); } [@@deriving qcheck, eq]
...
supportsReadMemoryRequest: (bool option  ); } [@@deriving qcheck, eq]
...
let exceptionBreakpointFilters (_t0 , (_t1 , (_t2 , _t3))) = Capabilities_0.exceptionBreakpointFilters _t0
...
let supportsStepInTargetsRequest (_t0 , (_t1 , (_t2 , _t3))) = Capabilities_10.supportsStepInTargetsRequest _t1
...
let additionalModuleColumns (_t0 , (_t1 , (_t2 , _t3))) = Capabilities_10.additionalModuleColumns _t1
let supportedChecksumAlgorithms (_t0 , (_t1 , (_t2 , _t3))) = Capabilities_10.supportedChecksumAlgorithms _t1
...
let supportTerminateDebuggee (_t0 , (_t1 , (_t2 , _t3))) = Capabilities_20.supportTerminateDebuggee _t2
let supportSuspendDebuggee (_t0 , (_t1 , (_t2 , _t3))) = Capabilities_20.supportSuspendDebuggee _t2
let supportsDelayedStackTraceLoading (_t0 , (_t1 , (_t2 , _t3))) = Capabilities_20.supportsDelayedStackTraceLoading _t2
...
let supportsWriteMemoryRequest (_t0 , (_t1 , (_t2 , _t3))) = Capabilities_30.supportsWriteMemoryRequest _t3
let supportsDisassembleRequest (_t0 , (_t1 , (_t2 , _t3))) = Capabilities_30.supportsDisassembleRequest _t3
let supportsCancelRequest (_t0 , (_t1 , (_t2 , _t3))) = Capabilities_30.supportsCancelRequest _t3
...
type t 
 ...
 type t = { capabilities: (Capabilities.t  ); } [@@deriving qcheck, eq]
...
val percentage : t -> (int option  )
 ...
 type t = { progressId: (string [@gen Gen.gen_utf8_str] );
title: (string [@gen Gen.gen_utf8_str] );
requestId: (int option [@gen Gen.gen_int31_opt] );
cancellable: (bool option  );
message: (string option [@gen Gen.gen_utf8_str_opt] );
percentage: (int option [@gen Gen.gen_int31_opt] ); } [@@deriving qcheck, eq]
...
val message : t -> (string option  )
 ...
 type t = { progressId: (string [@gen Gen.gen_utf8_str] );
message: (string option [@gen Gen.gen_utf8_str_opt] ); } [@@deriving qcheck, eq]
...
(* dont bother with a sig for enums, the inferred one is fine *)
 ...
 type t = All | Stacks | Threads | Variables | Other of string [@@deriving qcheck, eq]

 let enc = 
 let open Data_encoding in 
 conv_with_guard 
 (function All -> "all" | Stacks -> "stacks" | Threads -> "threads" | Variables -> "variables" | Other s -> s)
 (function "all" -> Ok All | "stacks" -> Ok Stacks | "threads" -> Ok Threads | "variables" -> Ok Variables | _ as s -> Ok (Other s))
 string
...
threadId: (int option [@gen Gen.gen_int31_opt] );
stackFrameId: (int option [@gen Gen.gen_int31_opt] ); } [@@deriving qcheck, eq]
...
val count : t -> (int  )
 ...
 type t = { memoryReference: (string [@gen Gen.gen_utf8_str] );
offset: (int [@gen Gen.gen_int31] );
count: (int [@gen Gen.gen_int31] ); } [@@deriving qcheck, eq]
...
(* dont bother with a sig for enums, the inferred one is fine *)
 ...
 type t = Integrated | External [@@deriving qcheck, eq]

 let enc = 
 let open Data_encoding in 
 conv_with_guard 
 (function Integrated -> "integrated" | External -> "external")
 (function "integrated" -> Ok Integrated | "external" -> Ok External | _ -> Error "RunInTerminalRequestArguments_kind")
 string
...
env: (Data_encoding.json option [@gen Gen.gen_json_opt] [@equal Eq.equal_json_opt]); } [@@deriving qcheck, eq]
...
val shellProcessId : t -> (int option  )
 ...
 type t = { processId: (int option [@gen Gen.gen_int31_opt] );
shellProcessId: (int option [@gen Gen.gen_int31_opt] ); } [@@deriving qcheck, eq]
...
(* dont bother with a sig for enums, the inferred one is fine *)
 ...
 type t = Path | Uri | Other of string [@@deriving qcheck, eq]
...
val supportsMemoryEvent : t -> (bool option  )
 ...
 type t = { clientID: (string option [@gen Gen.gen_utf8_str_opt] );
...
supportsRunInTerminalRequest: (bool option  ); } [@@deriving qcheck, eq]
...
supportsMemoryEvent: (bool option  ); } [@@deriving qcheck, eq]
...
let supportsMemoryEvent t = t.supportsMemoryEvent
 ...
 type t = (InitializeRequestArguments_0.t * InitializeRequestArguments_10.t) [@@deriving qcheck, eq]
...
let columnsStartAt1 (_t0 , _t1) = InitializeRequestArguments_0.columnsStartAt1 _t0
let pathFormat (_t0 , _t1) = InitializeRequestArguments_0.pathFormat _t0
let supportsVariableType (_t0 , _t1) = InitializeRequestArguments_0.supportsVariableType _t0
let supportsVariablePaging (_t0 , _t1) = InitializeRequestArguments_0.supportsVariablePaging _t0
let supportsRunInTerminalRequest (_t0 , _t1) = InitializeRequestArguments_0.supportsRunInTerminalRequest _t0
let supportsMemoryReferences (_t0 , _t1) = InitializeRequestArguments_10.supportsMemoryReferences _t1
let supportsProgressReporting (_t0 , _t1) = InitializeRequestArguments_10.supportsProgressReporting _t1
let supportsInvalidatedEvent (_t0 , _t1) = InitializeRequestArguments_10.supportsInvalidatedEvent _t1
let supportsMemoryEvent (_t0 , _t1) = InitializeRequestArguments_10.supportsMemoryEvent _t1
...
suspendDebuggee: (bool option  ); } [@@deriving qcheck, eq]
...
type t 
 ...
 type t = { restart: (bool option  ); } [@@deriving qcheck, eq]
...
column: (int option [@gen Gen.gen_int31_opt] );
endLine: (int option [@gen Gen.gen_int31_opt] );
endColumn: (int option [@gen Gen.gen_int31_opt] ); } [@@deriving qcheck, eq]
...
val endColumn : t -> (int option  )
 ...
 type t = { line: (int [@gen Gen.gen_int31] );
column: (int option [@gen Gen.gen_int31_opt] );
endLine: (int option [@gen Gen.gen_int31_opt] );
endColumn: (int option [@gen Gen.gen_int31_opt] ); } [@@deriving qcheck, eq]
...
type t 
 ...
 type t = { breakpoints: (BreakpointLocation.t list  ); } [@@deriving qcheck, eq]
...
logMessage: (string option [@gen Gen.gen_utf8_str_opt] ); } [@@deriving qcheck, eq]
...
sourceModified: (bool option  ); } [@@deriving qcheck, eq]
...
type t 
 ...
 type t = { breakpoints: (Breakpoint.t list  ); } [@@deriving qcheck, eq]
...
val hitCondition : t -> (string option  )
 ...
 type t = { name: (string [@gen Gen.gen_utf8_str] );
condition: (string option [@gen Gen.gen_utf8_str_opt] );
hitCondition: (string option [@gen Gen.gen_utf8_str_opt] ); } [@@deriving qcheck, eq]
...
type t 
 ...
 type t = { breakpoints: (FunctionBreakpoint.t list  ); } [@@deriving qcheck, eq]
...
val condition : t -> (string option  )
 ...
 type t = { filterId: (string [@gen Gen.gen_utf8_str] );
condition: (string option [@gen Gen.gen_utf8_str_opt] ); } [@@deriving qcheck, eq]
...
names: (string list [@gen Gen.gen_utf8_str_list] ); } [@@deriving qcheck, eq]
...
(* dont bother with a sig for enums, the inferred one is fine *)
 ...
 type t = Never | Always | Unhandled | UserUnhandled [@@deriving qcheck, eq]

 let enc = 
 let open Data_encoding in 
 conv_with_guard 
 (function Never -> "never" | Always -> "always" | Unhandled -> "unhandled" | UserUnhandled -> "userUnhandled")
 (function "never" -> Ok Never | "always" -> Ok Always | "unhandled" -> Ok Unhandled | "userUnhandled" -> Ok UserUnhandled | _ -> Error "ExceptionBreakMode")
 string
...
breakMode: (ExceptionBreakMode.t  ); } [@@deriving qcheck, eq]
...
val exceptionOptions : t -> (ExceptionOptions.t list option  )
 ...
 type t = { filters: (string list [@gen Gen.gen_utf8_str_list] );
filterOptions: (ExceptionFilterOptions.t list option  );
exceptionOptions: (ExceptionOptions.t list option  ); } [@@deriving qcheck, eq]
...
type t 
 ...
 type t = { breakpoints: (Breakpoint.t list option  ); } [@@deriving qcheck, eq]
...
val name : t -> (string  )
 ...
 type t = { variablesReference: (int option [@gen Gen.gen_int31_opt] );
name: (string [@gen Gen.gen_utf8_str] ); } [@@deriving qcheck, eq]
...
(* dont bother with a sig for enums, the inferred one is fine *)
 ...
 type t = Read | Write | ReadWrite [@@deriving qcheck, eq]

 let enc = 
 let open Data_encoding in 
 conv_with_guard 
 (function Read -> "read" | Write -> "write" | ReadWrite -> "readWrite")
 (function "read" -> Ok Read | "write" -> Ok Write | "readWrite" -> Ok ReadWrite | _ -> Error "DataBreakpointAccessType")
 string
...
val canPersist : t -> (bool option  )
 ...
 type t = { dataId: (string option [@gen Gen.gen_utf8_str_opt] );
description: (string [@gen Gen.gen_utf8_str] );
accessTypes: (DataBreakpointAccessType.t list option  );
canPersist: (bool option  ); } [@@deriving qcheck, eq]
...
val hitCondition : t -> (string option  )
 ...
 type t = { dataId: (string [@gen Gen.gen_utf8_str] );
...
type t 
 ...
 type t = { breakpoints: (DataBreakpoint.t list  ); } [@@deriving qcheck, eq]
...
val hitCondition : t -> (string option  )
 ...
 type t = { instructionReference: (string [@gen Gen.gen_utf8_str] );
...
type t 
 ...
 type t = { breakpoints: (InstructionBreakpoint.t list  ); } [@@deriving qcheck, eq]
...
val singleThread : t -> (bool option  )
 ...
 type t = { threadId: (int [@gen Gen.gen_int31] );
singleThread: (bool option  ); } [@@deriving qcheck, eq]
...
type t 
 ...
 type t = { allThreadsContinued: (bool option  ); } [@@deriving qcheck, eq]
...
(* dont bother with a sig for enums, the inferred one is fine *)
 ...
 type t = Statement | Line | Instruction [@@deriving qcheck, eq]

 let enc = 
 let open Data_encoding in 
 conv_with_guard 
 (function Statement -> "statement" | Line -> "line" | Instruction -> "instruction")
 (function "statement" -> Ok Statement | "line" -> Ok Line | "instruction" -> Ok Instruction | _ -> Error "SteppingGranularity")
 string
...
granularity: (SteppingGranularity.t option  ); } [@@deriving qcheck, eq]
...
type t 
 ...
 type t = { frameId: (int [@gen Gen.gen_int31] ); } [@@deriving qcheck, eq]
...
val targetId : t -> (int  )
 ...
 type t = { threadId: (int [@gen Gen.gen_int31] );
targetId: (int [@gen Gen.gen_int31] ); } [@@deriving qcheck, eq]
...
type t 
 ...
 type t = { threadId: (int [@gen Gen.gen_int31] ); } [@@deriving qcheck, eq]
...
format: (StackFrameFormat.t option  ); } [@@deriving qcheck, eq]
...
(* dont bother with a sig for enums, the inferred one is fine *)
 ...
 type t = Normal | Label | Subtle [@@deriving qcheck, eq]

 let enc = 
 let open Data_encoding in 
 conv_with_guard 
 (function Normal -> "normal" | Label -> "label" | Subtle -> "subtle")
 (function "normal" -> Ok Normal | "label" -> Ok Label | "subtle" -> Ok Subtle | _ -> Error "StackFrame_presentationHint")
 string
...
moduleId: (IntString.t option  ); } [@@deriving qcheck, eq]
...
module StackFrame_10 = struct 
 type t = { presentationHint: (StackFrame_presentationHint.t option  ); } [@@deriving qcheck, eq]
...
{presentationHint}
 ...
 type t = (StackFrame_0.t * StackFrame_10.t) [@@deriving qcheck, eq]
...
let instructionPointerReference (_t0 , _t1) = StackFrame_0.instructionPointerReference _t0
...
totalFrames: (int option [@gen Gen.gen_int31_opt] ); } [@@deriving qcheck, eq]
...
(* dont bother with a sig for enums, the inferred one is fine *)
 ...
 type t = Arguments | Locals | Registers | Other of string [@@deriving qcheck, eq]

 let enc = 
 let open Data_encoding in 
 conv_with_guard 
 (function Arguments -> "arguments" | Locals -> "locals" | Registers -> "registers" | Other s -> s)
 (function "arguments" -> Ok Arguments | "locals" -> Ok Locals | "registers" -> Ok Registers | _ as s -> Ok (Other s))
 string
...
line: (int option [@gen Gen.gen_int31_opt] );
column: (int option [@gen Gen.gen_int31_opt] );
endLine: (int option [@gen Gen.gen_int31_opt] ); } [@@deriving qcheck, eq]
...
module Scope_10 = struct 
 type t = { endColumn: (int option [@gen Gen.gen_int31_opt] ); } [@@deriving qcheck, eq]
...
{endColumn}
 ...
 type t = (Scope_0.t * Scope_10.t) [@@deriving qcheck, eq]
...
type t 
 ...
 type t = { scopes: (Scope.t list  ); } [@@deriving qcheck, eq]
...
(* dont bother with a sig for enums, the inferred one is fine *)
 ...
 type t = Indexed | Named [@@deriving qcheck, eq]

 let enc = 
 let open Data_encoding in 
 conv_with_guard 
 (function Indexed -> "indexed" | Named -> "named")
 (function "indexed" -> Ok Indexed | "named" -> Ok Named | _ -> Error "VariablesArguments_filter")
 string
...
(* dont bother with a sig for enums, the inferred one is fine *)
 ...
 type t = Property | Method | Class | Data | Event | BaseClass | InnerClass | Interface | MostDerivedClass | Virtual | DataBreakpoint | Other of string [@@deriving qcheck, eq]
 ...
 (function "property" -> Ok Property | "method" -> Ok Method | "class" -> Ok Class | "data" -> Ok Data | "event" -> Ok Event | "baseClass" -> Ok BaseClass | "innerClass" -> Ok InnerClass | "interface" -> Ok Interface | "mostDerivedClass" -> Ok MostDerivedClass | "virtual" -> Ok Virtual | "dataBreakpoint" -> Ok DataBreakpoint | _ as s -> Ok (Other s))
 string
...
(* dont bother with a sig for enums, the inferred one is fine *)
 ...
 type t = Static | Constant | ReadOnly | RawString | HasObjectId | CanHaveObjectId | HasSideEffects | HasDataBreakpoint | Other of string [@@deriving qcheck, eq]
 ...
 (function "static" -> Ok Static | "constant" -> Ok Constant | "readOnly" -> Ok ReadOnly | "rawString" -> Ok RawString | "hasObjectId" -> Ok HasObjectId | "canHaveObjectId" -> Ok CanHaveObjectId | "hasSideEffects" -> Ok HasSideEffects | "hasDataBreakpoint" -> Ok HasDataBreakpoint | _ as s -> Ok (Other s))
 string
...
(* dont bother with a sig for enums, the inferred one is fine *)
 ...
 type t = Public | Private | Protected | Internal | Final | Other of string [@@deriving qcheck, eq]

 let enc = 
 let open Data_encoding in 
 conv_with_guard 
 (function Public -> "public" | Private -> "private" | Protected -> "protected" | Internal -> "internal" | Final -> "final" | Other s -> s)
 (function "public" -> Ok Public | "private" -> Ok Private | "protected" -> Ok Protected | "internal" -> Ok Internal | "final" -> Ok Final | _ as s -> Ok (Other s))
 string
...
lazy_: (bool option  ); } [@@deriving qcheck, eq]
...
namedVariables: (int option [@gen Gen.gen_int31_opt] );
indexedVariables: (int option [@gen Gen.gen_int31_opt] );
memoryReference: (string option [@gen Gen.gen_utf8_str_opt] ); } [@@deriving qcheck, eq]
...
type t 
 ...
 type t = { variables: (Variable_.t list  ); } [@@deriving qcheck, eq]
...
val indexedVariables : t -> (int option  )
 ...
 type t = { value: (string [@gen Gen.gen_utf8_str] );
type_: (string option [@gen Gen.gen_utf8_str_opt] );
variablesReference: (int option [@gen Gen.gen_int31_opt] );
namedVariables: (int option [@gen Gen.gen_int31_opt] );
indexedVariables: (int option [@gen Gen.gen_int31_opt] ); } [@@deriving qcheck, eq]
...
sourceReference: (int [@gen Gen.gen_int31] ); } [@@deriving qcheck, eq]
...
val mimeType : t -> (string option  )
 ...
 type t = { content: (string [@gen Gen.gen_utf8_str] );
mimeType: (string option [@gen Gen.gen_utf8_str_opt] ); } [@@deriving qcheck, eq]
...
type t 
 ...
 type t = { threads: (Thread.t list  ); } [@@deriving qcheck, eq]
...
type t 
 ...
 type t = { threadIds: (int list option [@gen Gen.gen_int31_list_opt] ); } [@@deriving qcheck, eq]
...
val moduleCount : t -> (int option  )
 ...
 type t = { startModule: (int option [@gen Gen.gen_int31_opt] );
moduleCount: (int option [@gen Gen.gen_int31_opt] ); } [@@deriving qcheck, eq]
...
totalModules: (int option [@gen Gen.gen_int31_opt] ); } [@@deriving qcheck, eq]
...
type t 
 ...
 type t = { sources: (Source.t list  ); } [@@deriving qcheck, eq]
...
(* dont bother with a sig for enums, the inferred one is fine *)
 ...
 type t = Variables | Watch | Repl | Hover | Clipboard | Other of string [@@deriving qcheck, eq]

 let enc = 
 let open Data_encoding in 
 conv_with_guard 
 (function Variables -> "variables" | Watch -> "watch" | Repl -> "repl" | Hover -> "hover" | Clipboard -> "clipboard" | Other s -> s)
 (function "variables" -> Ok Variables | "watch" -> Ok Watch | "repl" -> Ok Repl | "hover" -> Ok Hover | "clipboard" -> Ok Clipboard | _ as s -> Ok (Other s))
 string
...
val format : t -> (ValueFormat.t option  )
 ...
 type t = { expression: (string [@gen Gen.gen_utf8_str] );
...
val memoryReference : t -> (string option  )
 ...
 type t = { result: (string [@gen Gen.gen_utf8_str] );
type_: (string option [@gen Gen.gen_utf8_str_opt] );
presentationHint: (VariablePresentationHint.t option  );
variablesReference: (int [@gen Gen.gen_int31] );
namedVariables: (int option [@gen Gen.gen_int31_opt] );
...
type t 
 ...
 type t = { targets: (StepInTarget.t list  ); } [@@deriving qcheck, eq]
...
val column : t -> (int option  )
 ...
 type t = { source: (Source.t  );
line: (int [@gen Gen.gen_int31] );
column: (int option [@gen Gen.gen_int31_opt] ); } [@@deriving qcheck, eq]
...
endLine: (int option [@gen Gen.gen_int31_opt] );
endColumn: (int option [@gen Gen.gen_int31_opt] );
instructionPointerReference: (string option [@gen Gen.gen_utf8_str_opt] ); } [@@deriving qcheck, eq]
...
type t 
 ...
 type t = { targets: (GotoTarget.t list  ); } [@@deriving qcheck, eq]
...
val line : t -> (int option  )
 ...
 type t = { frameId: (int option [@gen Gen.gen_int31_opt] );
text: (string [@gen Gen.gen_utf8_str] );
column: (int [@gen Gen.gen_int31] );
line: (int option [@gen Gen.gen_int31_opt] ); } [@@deriving qcheck, eq]
...
(* dont bother with a sig for enums, the inferred one is fine *)
 ...
 type t = Method | Function | Constructor | Field | Variable_ | Class | Interface | Module_ | Property | Unit | Value | Enum | Keyword | Snippet | Text | Color | File | Reference | Customcolor [@@deriving qcheck, eq]
 ...
 (function "method" -> Ok Method | "function" -> Ok Function | "constructor" -> Ok Constructor | "field" -> Ok Field | "variable" -> Ok Variable_ | "class" -> Ok Class | "interface" -> Ok Interface | "module" -> Ok Module_ | "property" -> Ok Property | "unit" -> Ok Unit | "value" -> Ok Value | "enum" -> Ok Enum | "keyword" -> Ok Keyword | "snippet" -> Ok Snippet | "text" -> Ok Text | "color" -> Ok Color | "file" -> Ok File | "reference" -> Ok Reference | "customcolor" -> Ok Customcolor | _ -> Error "CompletionItemType")
 string
...
val selectionLength : t -> (int option  )
 ...
 type t = { label: (string [@gen Gen.gen_utf8_str] );
text: (string option [@gen Gen.gen_utf8_str_opt] );
...
selectionLength: (int option [@gen Gen.gen_int31_opt] ); } [@@deriving qcheck, eq]
...
type t 
 ...
 type t = { targets: (CompletionItem.t list  ); } [@@deriving qcheck, eq]
...
val details : t -> (ExceptionDetails.t option  )
 ...
 type t = { exceptionId: (string [@gen Gen.gen_utf8_str] );
description: (string option [@gen Gen.gen_utf8_str_opt] );
breakMode: (ExceptionBreakMode.t  );
details: (ExceptionDetails.t option  ); } [@@deriving qcheck, eq]
...
val data : t -> (string option  )
 ...
 type t = { address: (string [@gen Gen.gen_utf8_str] );
unreadableBytes: (int option [@gen Gen.gen_int31_opt] );
data: (string option [@gen Gen.gen_utf8_str_opt] ); } [@@deriving qcheck, eq]
...
data: (string [@gen Gen.gen_utf8_str] ); } [@@deriving qcheck, eq]
...
val bytesWritten : t -> (int option  )
 ...
 type t = { offset: (int option [@gen Gen.gen_int31_opt] );
bytesWritten: (int option [@gen Gen.gen_int31_opt] ); } [@@deriving qcheck, eq]
...
resolveSymbols: (bool option  ); } [@@deriving qcheck, eq]
...
type t 
 ...
 type t = { instructions: (DisassembledInstruction.t list  ); } [@@deriving qcheck, eq]
...
type t 
 ...
 type t = { columns: (ColumnDescriptor.t list  ); } [@@deriving qcheck, eq]
...
| RunInTerminalRequest : (Dap_commands.runInTerminal, RunInTerminalRequestArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.runInTerminal, RunInTerminalRequestArguments.t, Presence.req) RequestMessage.t t
| InitializeRequest : (Dap_commands.initialize, InitializeRequestArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.initialize, InitializeRequestArguments.t, Presence.req) RequestMessage.t t
| ConfigurationDoneRequest : (Dap_commands.configurationDone, ConfigurationDoneArguments.t option, Presence.opt) RequestMessage.t -> (Dap_commands.configurationDone, ConfigurationDoneArguments.t option, Presence.opt) RequestMessage.t t
| LaunchRequest : (Dap_commands.launch, LaunchRequestArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.launch, LaunchRequestArguments.t, Presence.req) RequestMessage.t t
| AttachRequest : (Dap_commands.attach, AttachRequestArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.attach, AttachRequestArguments.t, Presence.req) RequestMessage.t t
| RestartRequest : (Dap_commands.restart, RestartArguments.t option, Presence.opt) RequestMessage.t -> (Dap_commands.restart, RestartArguments.t option, Presence.opt) RequestMessage.t t
| DisconnectRequest : (Dap_commands.disconnect, DisconnectArguments.t option, Presence.opt) RequestMessage.t -> (Dap_commands.disconnect, DisconnectArguments.t option, Presence.opt) RequestMessage.t t
| TerminateRequest : (Dap_commands.terminate, TerminateArguments.t option, Presence.opt) RequestMessage.t -> (Dap_commands.terminate, TerminateArguments.t option, Presence.opt) RequestMessage.t t
| BreakpointLocationsRequest : (Dap_commands.breakpointLocations, BreakpointLocationsArguments.t option, Presence.opt) RequestMessage.t -> (Dap_commands.breakpointLocations, BreakpointLocationsArguments.t option, Presence.opt) RequestMessage.t t
| SetBreakpointsRequest : (Dap_commands.setBreakpoints, SetBreakpointsArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.setBreakpoints, SetBreakpointsArguments.t, Presence.req) RequestMessage.t t
| SetFunctionBreakpointsRequest : (Dap_commands.setFunctionBreakpoints, SetFunctionBreakpointsArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.setFunctionBreakpoints, SetFunctionBreakpointsArguments.t, Presence.req) RequestMessage.t t
| SetExceptionBreakpointsRequest : (Dap_commands.setExceptionBreakpoints, SetExceptionBreakpointsArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.setExceptionBreakpoints, SetExceptionBreakpointsArguments.t, Presence.req) RequestMessage.t t
| DataBreakpointInfoRequest : (Dap_commands.dataBreakpointInfo, DataBreakpointInfoArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.dataBreakpointInfo, DataBreakpointInfoArguments.t, Presence.req) RequestMessage.t t
| SetDataBreakpointsRequest : (Dap_commands.setDataBreakpoints, SetDataBreakpointsArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.setDataBreakpoints, SetDataBreakpointsArguments.t, Presence.req) RequestMessage.t t
| SetInstructionBreakpointsRequest : (Dap_commands.setInstructionBreakpoints, SetInstructionBreakpointsArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.setInstructionBreakpoints, SetInstructionBreakpointsArguments.t, Presence.req) RequestMessage.t t
| ContinueRequest : (Dap_commands.continue, ContinueArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.continue, ContinueArguments.t, Presence.req) RequestMessage.t t
| NextRequest : (Dap_commands.next, NextArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.next, NextArguments.t, Presence.req) RequestMessage.t t
| StepInRequest : (Dap_commands.stepIn, StepInArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.stepIn, StepInArguments.t, Presence.req) RequestMessage.t t
| StepOutRequest : (Dap_commands.stepOut, StepOutArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.stepOut, StepOutArguments.t, Presence.req) RequestMessage.t t
| StepBackRequest : (Dap_commands.stepBack, StepBackArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.stepBack, StepBackArguments.t, Presence.req) RequestMessage.t t
| ReverseContinueRequest : (Dap_commands.reverseContinue, ReverseContinueArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.reverseContinue, ReverseContinueArguments.t, Presence.req) RequestMessage.t t
| RestartFrameRequest : (Dap_commands.restartFrame, RestartFrameArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.restartFrame, RestartFrameArguments.t, Presence.req) RequestMessage.t t
| GotoRequest : (Dap_commands.goto, GotoArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.goto, GotoArguments.t, Presence.req) RequestMessage.t t
| PauseRequest : (Dap_commands.pause, PauseArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.pause, PauseArguments.t, Presence.req) RequestMessage.t t
| StackTraceRequest : (Dap_commands.stackTrace, StackTraceArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.stackTrace, StackTraceArguments.t, Presence.req) RequestMessage.t t
| ScopesRequest : (Dap_commands.scopes, ScopesArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.scopes, ScopesArguments.t, Presence.req) RequestMessage.t t
| VariablesRequest : (Dap_commands.variables, VariablesArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.variables, VariablesArguments.t, Presence.req) RequestMessage.t t
| SetVariableRequest : (Dap_commands.setVariable, SetVariableArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.setVariable, SetVariableArguments.t, Presence.req) RequestMessage.t t
| SourceRequest : (Dap_commands.source, SourceArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.source, SourceArguments.t, Presence.req) RequestMessage.t t
| ThreadsRequest : (Dap_commands.threads, EmptyObject.t option, Presence.opt) RequestMessage.t -> (Dap_commands.threads, EmptyObject.t option, Presence.opt) RequestMessage.t t
| TerminateThreadsRequest : (Dap_commands.terminateThreads, TerminateThreadsArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.terminateThreads, TerminateThreadsArguments.t, Presence.req) RequestMessage.t t
| ModulesRequest : (Dap_commands.modules, ModulesArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.modules, ModulesArguments.t, Presence.req) RequestMessage.t t
| LoadedSourcesRequest : (Dap_commands.loadedSources, LoadedSourcesArguments.t option, Presence.opt) RequestMessage.t -> (Dap_commands.loadedSources, LoadedSourcesArguments.t option, Presence.opt) RequestMessage.t t
| EvaluateRequest : (Dap_commands.evaluate, EvaluateArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.evaluate, EvaluateArguments.t, Presence.req) RequestMessage.t t
| SetExpressionRequest : (Dap_commands.setExpression, SetExpressionArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.setExpression, SetExpressionArguments.t, Presence.req) RequestMessage.t t
| StepInTargetsRequest : (Dap_commands.stepInTargets, StepInTargetsArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.stepInTargets, StepInTargetsArguments.t, Presence.req) RequestMessage.t t
| GotoTargetsRequest : (Dap_commands.gotoTargets, GotoTargetsArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.gotoTargets, GotoTargetsArguments.t, Presence.req) RequestMessage.t t
| CompletionsRequest : (Dap_commands.completions, CompletionsArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.completions, CompletionsArguments.t, Presence.req) RequestMessage.t t
| ExceptionInfoRequest : (Dap_commands.exceptionInfo, ExceptionInfoArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.exceptionInfo, ExceptionInfoArguments.t, Presence.req) RequestMessage.t t
| ReadMemoryRequest : (Dap_commands.readMemory, ReadMemoryArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.readMemory, ReadMemoryArguments.t, Presence.req) RequestMessage.t t
| WriteMemoryRequest : (Dap_commands.writeMemory, WriteMemoryArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.writeMemory, WriteMemoryArguments.t, Presence.req) RequestMessage.t t
| DisassembleRequest : (Dap_commands.disassemble, DisassembleArguments.t, Presence.req) RequestMessage.t -> (Dap_commands.disassemble, DisassembleArguments.t, Presence.req) RequestMessage.t t
...
let nextRequest req = NextRequest req
let stepInRequest req = StepInRequest req
let stepOutRequest req = StepOutRequest req
let stepBackRequest req = StepBackRequest req
let reverseContinueRequest req = ReverseContinueRequest req
let restartFrameRequest req = RestartFrameRequest req
let gotoRequest req = GotoRequest req
let pauseRequest req = PauseRequest req
let stackTraceRequest req = StackTraceRequest req
let scopesRequest req = ScopesRequest req
let variablesRequest req = VariablesRequest req
let setVariableRequest req = SetVariableRequest req
let sourceRequest req = SourceRequest req
let threadsRequest req = ThreadsRequest req
let terminateThreadsRequest req = TerminateThreadsRequest req
let modulesRequest req = ModulesRequest req
let loadedSourcesRequest req = LoadedSourcesRequest req
...
let gen_runInTerminalRequest = (runInTerminalRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.runInTerminal ~arguments ()) @@ tup2 Gen.gen_int31 RunInTerminalRequestArguments.gen ), RequestMessage.enc Dap_commands.runInTerminal RunInTerminalRequestArguments.enc)
let gen_initializeRequest = (initializeRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.initialize ~arguments ()) @@ tup2 Gen.gen_int31 InitializeRequestArguments.gen ), RequestMessage.enc Dap_commands.initialize InitializeRequestArguments.enc)
let gen_configurationDoneRequest = (configurationDoneRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make_opt ~seq ~command:Dap_commands.configurationDone ~arguments ()) @@ tup2 Gen.gen_int31 ConfigurationDoneArguments.gen ), RequestMessage.enc_opt Dap_commands.configurationDone ConfigurationDoneArguments.enc)
let gen_launchRequest = (launchRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.launch ~arguments ()) @@ tup2 Gen.gen_int31 LaunchRequestArguments.gen ), RequestMessage.enc Dap_commands.launch LaunchRequestArguments.enc)
let gen_attachRequest = (attachRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.attach ~arguments ()) @@ tup2 Gen.gen_int31 AttachRequestArguments.gen ), RequestMessage.enc Dap_commands.attach AttachRequestArguments.enc)
let gen_restartRequest = (restartRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make_opt ~seq ~command:Dap_commands.restart ~arguments ()) @@ tup2 Gen.gen_int31 RestartArguments.gen ), RequestMessage.enc_opt Dap_commands.restart RestartArguments.enc)
let gen_disconnectRequest = (disconnectRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make_opt ~seq ~command:Dap_commands.disconnect ~arguments ()) @@ tup2 Gen.gen_int31 DisconnectArguments.gen ), RequestMessage.enc_opt Dap_commands.disconnect DisconnectArguments.enc)
let gen_terminateRequest = (terminateRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make_opt ~seq ~command:Dap_commands.terminate ~arguments ()) @@ tup2 Gen.gen_int31 TerminateArguments.gen ), RequestMessage.enc_opt Dap_commands.terminate TerminateArguments.enc)
let gen_breakpointLocationsRequest = (breakpointLocationsRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make_opt ~seq ~command:Dap_commands.breakpointLocations ~arguments ()) @@ tup2 Gen.gen_int31 BreakpointLocationsArguments.gen ), RequestMessage.enc_opt Dap_commands.breakpointLocations BreakpointLocationsArguments.enc)
let gen_setBreakpointsRequest = (setBreakpointsRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.setBreakpoints ~arguments ()) @@ tup2 Gen.gen_int31 SetBreakpointsArguments.gen ), RequestMessage.enc Dap_commands.setBreakpoints SetBreakpointsArguments.enc)
let gen_setFunctionBreakpointsRequest = (setFunctionBreakpointsRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.setFunctionBreakpoints ~arguments ()) @@ tup2 Gen.gen_int31 SetFunctionBreakpointsArguments.gen ), RequestMessage.enc Dap_commands.setFunctionBreakpoints SetFunctionBreakpointsArguments.enc)
let gen_setExceptionBreakpointsRequest = (setExceptionBreakpointsRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.setExceptionBreakpoints ~arguments ()) @@ tup2 Gen.gen_int31 SetExceptionBreakpointsArguments.gen ), RequestMessage.enc Dap_commands.setExceptionBreakpoints SetExceptionBreakpointsArguments.enc)
let gen_dataBreakpointInfoRequest = (dataBreakpointInfoRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.dataBreakpointInfo ~arguments ()) @@ tup2 Gen.gen_int31 DataBreakpointInfoArguments.gen ), RequestMessage.enc Dap_commands.dataBreakpointInfo DataBreakpointInfoArguments.enc)
let gen_setDataBreakpointsRequest = (setDataBreakpointsRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.setDataBreakpoints ~arguments ()) @@ tup2 Gen.gen_int31 SetDataBreakpointsArguments.gen ), RequestMessage.enc Dap_commands.setDataBreakpoints SetDataBreakpointsArguments.enc)
let gen_setInstructionBreakpointsRequest = (setInstructionBreakpointsRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.setInstructionBreakpoints ~arguments ()) @@ tup2 Gen.gen_int31 SetInstructionBreakpointsArguments.gen ), RequestMessage.enc Dap_commands.setInstructionBreakpoints SetInstructionBreakpointsArguments.enc)
let gen_continueRequest = (continueRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.continue ~arguments ()) @@ tup2 Gen.gen_int31 ContinueArguments.gen ), RequestMessage.enc Dap_commands.continue ContinueArguments.enc)
let gen_nextRequest = (nextRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.next ~arguments ()) @@ tup2 Gen.gen_int31 NextArguments.gen ), RequestMessage.enc Dap_commands.next NextArguments.enc)
let gen_stepInRequest = (stepInRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.stepIn ~arguments ()) @@ tup2 Gen.gen_int31 StepInArguments.gen ), RequestMessage.enc Dap_commands.stepIn StepInArguments.enc)
let gen_stepOutRequest = (stepOutRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.stepOut ~arguments ()) @@ tup2 Gen.gen_int31 StepOutArguments.gen ), RequestMessage.enc Dap_commands.stepOut StepOutArguments.enc)
let gen_stepBackRequest = (stepBackRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.stepBack ~arguments ()) @@ tup2 Gen.gen_int31 StepBackArguments.gen ), RequestMessage.enc Dap_commands.stepBack StepBackArguments.enc)
let gen_reverseContinueRequest = (reverseContinueRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.reverseContinue ~arguments ()) @@ tup2 Gen.gen_int31 ReverseContinueArguments.gen ), RequestMessage.enc Dap_commands.reverseContinue ReverseContinueArguments.enc)
let gen_restartFrameRequest = (restartFrameRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.restartFrame ~arguments ()) @@ tup2 Gen.gen_int31 RestartFrameArguments.gen ), RequestMessage.enc Dap_commands.restartFrame RestartFrameArguments.enc)
let gen_gotoRequest = (gotoRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.goto ~arguments ()) @@ tup2 Gen.gen_int31 GotoArguments.gen ), RequestMessage.enc Dap_commands.goto GotoArguments.enc)
let gen_pauseRequest = (pauseRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.pause ~arguments ()) @@ tup2 Gen.gen_int31 PauseArguments.gen ), RequestMessage.enc Dap_commands.pause PauseArguments.enc)
let gen_stackTraceRequest = (stackTraceRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.stackTrace ~arguments ()) @@ tup2 Gen.gen_int31 StackTraceArguments.gen ), RequestMessage.enc Dap_commands.stackTrace StackTraceArguments.enc)
let gen_scopesRequest = (scopesRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.scopes ~arguments ()) @@ tup2 Gen.gen_int31 ScopesArguments.gen ), RequestMessage.enc Dap_commands.scopes ScopesArguments.enc)
let gen_variablesRequest = (variablesRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.variables ~arguments ()) @@ tup2 Gen.gen_int31 VariablesArguments.gen ), RequestMessage.enc Dap_commands.variables VariablesArguments.enc)
let gen_setVariableRequest = (setVariableRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.setVariable ~arguments ()) @@ tup2 Gen.gen_int31 SetVariableArguments.gen ), RequestMessage.enc Dap_commands.setVariable SetVariableArguments.enc)
let gen_sourceRequest = (sourceRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.source ~arguments ()) @@ tup2 Gen.gen_int31 SourceArguments.gen ), RequestMessage.enc Dap_commands.source SourceArguments.enc)
let gen_threadsRequest = (threadsRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make_opt ~seq ~command:Dap_commands.threads ~arguments ()) @@ tup2 Gen.gen_int31 EmptyObject.gen ), RequestMessage.enc_opt Dap_commands.threads EmptyObject.enc)
let gen_terminateThreadsRequest = (terminateThreadsRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.terminateThreads ~arguments ()) @@ tup2 Gen.gen_int31 TerminateThreadsArguments.gen ), RequestMessage.enc Dap_commands.terminateThreads TerminateThreadsArguments.enc)
let gen_modulesRequest = (modulesRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.modules ~arguments ()) @@ tup2 Gen.gen_int31 ModulesArguments.gen ), RequestMessage.enc Dap_commands.modules ModulesArguments.enc)
let gen_loadedSourcesRequest = (loadedSourcesRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make_opt ~seq ~command:Dap_commands.loadedSources ~arguments ()) @@ tup2 Gen.gen_int31 LoadedSourcesArguments.gen ), RequestMessage.enc_opt Dap_commands.loadedSources LoadedSourcesArguments.enc)
let gen_evaluateRequest = (evaluateRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.evaluate ~arguments ()) @@ tup2 Gen.gen_int31 EvaluateArguments.gen ), RequestMessage.enc Dap_commands.evaluate EvaluateArguments.enc)
let gen_setExpressionRequest = (setExpressionRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.setExpression ~arguments ()) @@ tup2 Gen.gen_int31 SetExpressionArguments.gen ), RequestMessage.enc Dap_commands.setExpression SetExpressionArguments.enc)
let gen_stepInTargetsRequest = (stepInTargetsRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.stepInTargets ~arguments ()) @@ tup2 Gen.gen_int31 StepInTargetsArguments.gen ), RequestMessage.enc Dap_commands.stepInTargets StepInTargetsArguments.enc)
let gen_gotoTargetsRequest = (gotoTargetsRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.gotoTargets ~arguments ()) @@ tup2 Gen.gen_int31 GotoTargetsArguments.gen ), RequestMessage.enc Dap_commands.gotoTargets GotoTargetsArguments.enc)
let gen_completionsRequest = (completionsRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.completions ~arguments ()) @@ tup2 Gen.gen_int31 CompletionsArguments.gen ), RequestMessage.enc Dap_commands.completions CompletionsArguments.enc)
let gen_exceptionInfoRequest = (exceptionInfoRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.exceptionInfo ~arguments ()) @@ tup2 Gen.gen_int31 ExceptionInfoArguments.gen ), RequestMessage.enc Dap_commands.exceptionInfo ExceptionInfoArguments.enc)
let gen_readMemoryRequest = (readMemoryRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.readMemory ~arguments ()) @@ tup2 Gen.gen_int31 ReadMemoryArguments.gen ), RequestMessage.enc Dap_commands.readMemory ReadMemoryArguments.enc)
let gen_writeMemoryRequest = (writeMemoryRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.writeMemory ~arguments ()) @@ tup2 Gen.gen_int31 WriteMemoryArguments.gen ), RequestMessage.enc Dap_commands.writeMemory WriteMemoryArguments.enc)
let gen_disassembleRequest = (disassembleRequest, QCheck.Gen.( map (fun (seq, arguments) -> RequestMessage.make ~seq ~command:Dap_commands.disassemble ~arguments ()) @@ tup2 Gen.gen_int31 DisassembleArguments.gen ), RequestMessage.enc Dap_commands.disassemble DisassembleArguments.enc)
...
| CancelResponse : (Dap_commands.cancel, EmptyObject.t option, Presence.opt) ResponseMessage.t -> (Dap_commands.cancel, EmptyObject.t option, Presence.opt) ResponseMessage.t t
| RunInTerminalResponse : (Dap_commands.runInTerminal, RunInTerminalResponse_body.t, Presence.req) ResponseMessage.t -> (Dap_commands.runInTerminal, RunInTerminalResponse_body.t, Presence.req) ResponseMessage.t t
| InitializeResponse : (Dap_commands.initialize, Capabilities.t option, Presence.opt) ResponseMessage.t -> (Dap_commands.initialize, Capabilities.t option, Presence.opt) ResponseMessage.t t
| ConfigurationDoneResponse : (Dap_commands.configurationDone, EmptyObject.t option, Presence.opt) ResponseMessage.t -> (Dap_commands.configurationDone, EmptyObject.t option, Presence.opt) ResponseMessage.t t
| LaunchResponse : (Dap_commands.launch, EmptyObject.t option, Presence.opt) ResponseMessage.t -> (Dap_commands.launch, EmptyObject.t option, Presence.opt) ResponseMessage.t t
| AttachResponse : (Dap_commands.attach, EmptyObject.t option, Presence.opt) ResponseMessage.t -> (Dap_commands.attach, EmptyObject.t option, Presence.opt) ResponseMessage.t t
| RestartResponse : (Dap_commands.restart, EmptyObject.t option, Presence.opt) ResponseMessage.t -> (Dap_commands.restart, EmptyObject.t option, Presence.opt) ResponseMessage.t t
| DisconnectResponse : (Dap_commands.disconnect, EmptyObject.t option, Presence.opt) ResponseMessage.t -> (Dap_commands.disconnect, EmptyObject.t option, Presence.opt) ResponseMessage.t t
| TerminateResponse : (Dap_commands.terminate, EmptyObject.t option, Presence.opt) ResponseMessage.t -> (Dap_commands.terminate, EmptyObject.t option, Presence.opt) ResponseMessage.t t
| BreakpointLocationsResponse : (Dap_commands.breakpointLocations, BreakpointLocationsResponse_body.t, Presence.req) ResponseMessage.t -> (Dap_commands.breakpointLocations, BreakpointLocationsResponse_body.t, Presence.req) ResponseMessage.t t
| SetBreakpointsResponse : (Dap_commands.setBreakpoints, SetBreakpointsResponse_body.t, Presence.req) ResponseMessage.t -> (Dap_commands.setBreakpoints, SetBreakpointsResponse_body.t, Presence.req) ResponseMessage.t t
| SetFunctionBreakpointsResponse : (Dap_commands.setFunctionBreakpoints, SetFunctionBreakpointsResponse_body.t, Presence.req) ResponseMessage.t -> (Dap_commands.setFunctionBreakpoints, SetFunctionBreakpointsResponse_body.t, Presence.req) ResponseMessage.t t
| SetExceptionBreakpointsResponse : (Dap_commands.setExceptionBreakpoints, SetExceptionBreakpointsResponse_body.t option, Presence.opt) ResponseMessage.t -> (Dap_commands.setExceptionBreakpoints, SetExceptionBreakpointsResponse_body.t option, Presence.opt) ResponseMessage.t t
| DataBreakpointInfoResponse : (Dap_commands.dataBreakpointInfo, DataBreakpointInfoResponse_body.t, Presence.req) ResponseMessage.t -> (Dap_commands.dataBreakpointInfo, DataBreakpointInfoResponse_body.t, Presence.req) ResponseMessage.t t
| SetDataBreakpointsResponse : (Dap_commands.setDataBreakpoints, SetDataBreakpointsResponse_body.t, Presence.req) ResponseMessage.t -> (Dap_commands.setDataBreakpoints, SetDataBreakpointsResponse_body.t, Presence.req) ResponseMessage.t t
| SetInstructionBreakpointsResponse : (Dap_commands.setInstructionBreakpoints, SetInstructionBreakpointsResponse_body.t, Presence.req) ResponseMessage.t -> (Dap_commands.setInstructionBreakpoints, SetInstructionBreakpointsResponse_body.t, Presence.req) ResponseMessage.t t
| ContinueResponse : (Dap_commands.continue, ContinueResponse_body.t, Presence.req) ResponseMessage.t -> (Dap_commands.continue, ContinueResponse_body.t, Presence.req) ResponseMessage.t t
| NextResponse : (Dap_commands.next, EmptyObject.t option, Presence.opt) ResponseMessage.t -> (Dap_commands.next, EmptyObject.t option, Presence.opt) ResponseMessage.t t
| StepInResponse : (Dap_commands.stepIn, EmptyObject.t option, Presence.opt) ResponseMessage.t -> (Dap_commands.stepIn, EmptyObject.t option, Presence.opt) ResponseMessage.t t
| StepOutResponse : (Dap_commands.stepOut, EmptyObject.t option, Presence.opt) ResponseMessage.t -> (Dap_commands.stepOut, EmptyObject.t option, Presence.opt) ResponseMessage.t t
| StepBackResponse : (Dap_commands.stepBack, EmptyObject.t option, Presence.opt) ResponseMessage.t -> (Dap_commands.stepBack, EmptyObject.t option, Presence.opt) ResponseMessage.t t
| ReverseContinueResponse : (Dap_commands.reverseContinue, EmptyObject.t option, Presence.opt) ResponseMessage.t -> (Dap_commands.reverseContinue, EmptyObject.t option, Presence.opt) ResponseMessage.t t
| RestartFrameResponse : (Dap_commands.restartFrame, EmptyObject.t option, Presence.opt) ResponseMessage.t -> (Dap_commands.restartFrame, EmptyObject.t option, Presence.opt) ResponseMessage.t t
| GotoResponse : (Dap_commands.goto, EmptyObject.t option, Presence.opt) ResponseMessage.t -> (Dap_commands.goto, EmptyObject.t option, Presence.opt) ResponseMessage.t t
| PauseResponse : (Dap_commands.pause, EmptyObject.t option, Presence.opt) ResponseMessage.t -> (Dap_commands.pause, EmptyObject.t option, Presence.opt) ResponseMessage.t t
| StackTraceResponse : (Dap_commands.stackTrace, StackTraceResponse_body.t, Presence.req) ResponseMessage.t -> (Dap_commands.stackTrace, StackTraceResponse_body.t, Presence.req) ResponseMessage.t t
| ScopesResponse : (Dap_commands.scopes, ScopesResponse_body.t, Presence.req) ResponseMessage.t -> (Dap_commands.scopes, ScopesResponse_body.t, Presence.req) ResponseMessage.t t
| VariablesResponse : (Dap_commands.variables, VariablesResponse_body.t, Presence.req) ResponseMessage.t -> (Dap_commands.variables, VariablesResponse_body.t, Presence.req) ResponseMessage.t t
| SetVariableResponse : (Dap_commands.setVariable, SetVariableResponse_body.t, Presence.req) ResponseMessage.t -> (Dap_commands.setVariable, SetVariableResponse_body.t, Presence.req) ResponseMessage.t t
| SourceResponse : (Dap_commands.source, SourceResponse_body.t, Presence.req) ResponseMessage.t -> (Dap_commands.source, SourceResponse_body.t, Presence.req) ResponseMessage.t t
| ThreadsResponse : (Dap_commands.threads, ThreadsResponse_body.t, Presence.req) ResponseMessage.t -> (Dap_commands.threads, ThreadsResponse_body.t, Presence.req) ResponseMessage.t t
| TerminateThreadsResponse : (Dap_commands.terminateThreads, EmptyObject.t option, Presence.opt) ResponseMessage.t -> (Dap_commands.terminateThreads, EmptyObject.t option, Presence.opt) ResponseMessage.t t
| ModulesResponse : (Dap_commands.modules, ModulesResponse_body.t, Presence.req) ResponseMessage.t -> (Dap_commands.modules, ModulesResponse_body.t, Presence.req) ResponseMessage.t t
| LoadedSourcesResponse : (Dap_commands.loadedSources, LoadedSourcesResponse_body.t, Presence.req) ResponseMessage.t -> (Dap_commands.loadedSources, LoadedSourcesResponse_body.t, Presence.req) ResponseMessage.t t
| EvaluateResponse : (Dap_commands.evaluate, EvaluateResponse_body.t, Presence.req) ResponseMessage.t -> (Dap_commands.evaluate, EvaluateResponse_body.t, Presence.req) ResponseMessage.t t
| SetExpressionResponse : (Dap_commands.setExpression, SetExpressionResponse_body.t, Presence.req) ResponseMessage.t -> (Dap_commands.setExpression, SetExpressionResponse_body.t, Presence.req) ResponseMessage.t t
| StepInTargetsResponse : (Dap_commands.stepInTargets, StepInTargetsResponse_body.t, Presence.req) ResponseMessage.t -> (Dap_commands.stepInTargets, StepInTargetsResponse_body.t, Presence.req) ResponseMessage.t t
| GotoTargetsResponse : (Dap_commands.gotoTargets, GotoTargetsResponse_body.t, Presence.req) ResponseMessage.t -> (Dap_commands.gotoTargets, GotoTargetsResponse_body.t, Presence.req) ResponseMessage.t t
| CompletionsResponse : (Dap_commands.completions, CompletionsResponse_body.t, Presence.req) ResponseMessage.t -> (Dap_commands.completions, CompletionsResponse_body.t, Presence.req) ResponseMessage.t t
| ExceptionInfoResponse : (Dap_commands.exceptionInfo, ExceptionInfoResponse_body.t, Presence.req) ResponseMessage.t -> (Dap_commands.exceptionInfo, ExceptionInfoResponse_body.t, Presence.req) ResponseMessage.t t
| ReadMemoryResponse : (Dap_commands.readMemory, ReadMemoryResponse_body.t option, Presence.opt) ResponseMessage.t -> (Dap_commands.readMemory, ReadMemoryResponse_body.t option, Presence.opt) ResponseMessage.t t
| WriteMemoryResponse : (Dap_commands.writeMemory, WriteMemoryResponse_body.t option, Presence.opt) ResponseMessage.t -> (Dap_commands.writeMemory, WriteMemoryResponse_body.t option, Presence.opt) ResponseMessage.t t
| DisassembleResponse : (Dap_commands.disassemble, DisassembleResponse_body.t option, Presence.opt) ResponseMessage.t -> (Dap_commands.disassemble, DisassembleResponse_body.t option, Presence.opt) ResponseMessage.t t

 (* response constructors *) let errorResponse resp = ErrorResponse resp
let cancelResponse resp = CancelResponse resp
let runInTerminalResponse resp = RunInTerminalResponse resp
let initializeResponse resp = InitializeResponse resp
let configurationDoneResponse resp = ConfigurationDoneResponse resp
let launchResponse resp = LaunchResponse resp
let attachResponse resp = AttachResponse resp
let restartResponse resp = RestartResponse resp
let disconnectResponse resp = DisconnectResponse resp
let terminateResponse resp = TerminateResponse resp
let breakpointLocationsResponse resp = BreakpointLocationsResponse resp
let setBreakpointsResponse resp = SetBreakpointsResponse resp
let setFunctionBreakpointsResponse resp = SetFunctionBreakpointsResponse resp
let setExceptionBreakpointsResponse resp = SetExceptionBreakpointsResponse resp
let dataBreakpointInfoResponse resp = DataBreakpointInfoResponse resp
let setDataBreakpointsResponse resp = SetDataBreakpointsResponse resp
let setInstructionBreakpointsResponse resp = SetInstructionBreakpointsResponse resp
let continueResponse resp = ContinueResponse resp
let nextResponse resp = NextResponse resp
let stepInResponse resp = StepInResponse resp
let stepOutResponse resp = StepOutResponse resp
let stepBackResponse resp = StepBackResponse resp
let reverseContinueResponse resp = ReverseContinueResponse resp
let restartFrameResponse resp = RestartFrameResponse resp
let gotoResponse resp = GotoResponse resp
let pauseResponse resp = PauseResponse resp
let stackTraceResponse resp = StackTraceResponse resp
let scopesResponse resp = ScopesResponse resp
let variablesResponse resp = VariablesResponse resp
let setVariableResponse resp = SetVariableResponse resp
let sourceResponse resp = SourceResponse resp
let threadsResponse resp = ThreadsResponse resp
let terminateThreadsResponse resp = TerminateThreadsResponse resp
let modulesResponse resp = ModulesResponse resp
let loadedSourcesResponse resp = LoadedSourcesResponse resp
let evaluateResponse resp = EvaluateResponse resp
let setExpressionResponse resp = SetExpressionResponse resp
let stepInTargetsResponse resp = StepInTargetsResponse resp
let gotoTargetsResponse resp = GotoTargetsResponse resp
let completionsResponse resp = CompletionsResponse resp
let exceptionInfoResponse resp = ExceptionInfoResponse resp
let readMemoryResponse resp = ReadMemoryResponse resp
let writeMemoryResponse resp = WriteMemoryResponse resp
let disassembleResponse resp = DisassembleResponse resp
...
let gen_cancelResponse = (cancelResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make_opt ~seq ~request_seq ~success ~command:Dap_commands.cancel ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt EmptyObject.gen ), ResponseMessage.enc_opt Dap_commands.cancel EmptyObject.enc)
let gen_runInTerminalResponse = (runInTerminalResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make ~seq ~request_seq ~success ~command:Dap_commands.runInTerminal ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt RunInTerminalResponse_body.gen ), ResponseMessage.enc Dap_commands.runInTerminal RunInTerminalResponse_body.enc)
let gen_initializeResponse = (initializeResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make_opt ~seq ~request_seq ~success ~command:Dap_commands.initialize ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt Capabilities.gen ), ResponseMessage.enc_opt Dap_commands.initialize Capabilities.enc)
let gen_configurationDoneResponse = (configurationDoneResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make_opt ~seq ~request_seq ~success ~command:Dap_commands.configurationDone ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt EmptyObject.gen ), ResponseMessage.enc_opt Dap_commands.configurationDone EmptyObject.enc)
let gen_launchResponse = (launchResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make_opt ~seq ~request_seq ~success ~command:Dap_commands.launch ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt EmptyObject.gen ), ResponseMessage.enc_opt Dap_commands.launch EmptyObject.enc)
let gen_attachResponse = (attachResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make_opt ~seq ~request_seq ~success ~command:Dap_commands.attach ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt EmptyObject.gen ), ResponseMessage.enc_opt Dap_commands.attach EmptyObject.enc)
let gen_restartResponse = (restartResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make_opt ~seq ~request_seq ~success ~command:Dap_commands.restart ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt EmptyObject.gen ), ResponseMessage.enc_opt Dap_commands.restart EmptyObject.enc)
let gen_disconnectResponse = (disconnectResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make_opt ~seq ~request_seq ~success ~command:Dap_commands.disconnect ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt EmptyObject.gen ), ResponseMessage.enc_opt Dap_commands.disconnect EmptyObject.enc)
let gen_terminateResponse = (terminateResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make_opt ~seq ~request_seq ~success ~command:Dap_commands.terminate ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt EmptyObject.gen ), ResponseMessage.enc_opt Dap_commands.terminate EmptyObject.enc)
let gen_breakpointLocationsResponse = (breakpointLocationsResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make ~seq ~request_seq ~success ~command:Dap_commands.breakpointLocations ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt BreakpointLocationsResponse_body.gen ), ResponseMessage.enc Dap_commands.breakpointLocations BreakpointLocationsResponse_body.enc)
let gen_setBreakpointsResponse = (setBreakpointsResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make ~seq ~request_seq ~success ~command:Dap_commands.setBreakpoints ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt SetBreakpointsResponse_body.gen ), ResponseMessage.enc Dap_commands.setBreakpoints SetBreakpointsResponse_body.enc)
let gen_setFunctionBreakpointsResponse = (setFunctionBreakpointsResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make ~seq ~request_seq ~success ~command:Dap_commands.setFunctionBreakpoints ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt SetFunctionBreakpointsResponse_body.gen ), ResponseMessage.enc Dap_commands.setFunctionBreakpoints SetFunctionBreakpointsResponse_body.enc)
let gen_setExceptionBreakpointsResponse = (setExceptionBreakpointsResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make_opt ~seq ~request_seq ~success ~command:Dap_commands.setExceptionBreakpoints ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt SetExceptionBreakpointsResponse_body.gen ), ResponseMessage.enc_opt Dap_commands.setExceptionBreakpoints SetExceptionBreakpointsResponse_body.enc)
let gen_dataBreakpointInfoResponse = (dataBreakpointInfoResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make ~seq ~request_seq ~success ~command:Dap_commands.dataBreakpointInfo ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt DataBreakpointInfoResponse_body.gen ), ResponseMessage.enc Dap_commands.dataBreakpointInfo DataBreakpointInfoResponse_body.enc)
let gen_setDataBreakpointsResponse = (setDataBreakpointsResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make ~seq ~request_seq ~success ~command:Dap_commands.setDataBreakpoints ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt SetDataBreakpointsResponse_body.gen ), ResponseMessage.enc Dap_commands.setDataBreakpoints SetDataBreakpointsResponse_body.enc)
let gen_setInstructionBreakpointsResponse = (setInstructionBreakpointsResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make ~seq ~request_seq ~success ~command:Dap_commands.setInstructionBreakpoints ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt SetInstructionBreakpointsResponse_body.gen ), ResponseMessage.enc Dap_commands.setInstructionBreakpoints SetInstructionBreakpointsResponse_body.enc)
let gen_continueResponse = (continueResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make ~seq ~request_seq ~success ~command:Dap_commands.continue ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt ContinueResponse_body.gen ), ResponseMessage.enc Dap_commands.continue ContinueResponse_body.enc)
let gen_nextResponse = (nextResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make_opt ~seq ~request_seq ~success ~command:Dap_commands.next ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt EmptyObject.gen ), ResponseMessage.enc_opt Dap_commands.next EmptyObject.enc)
let gen_stepInResponse = (stepInResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make_opt ~seq ~request_seq ~success ~command:Dap_commands.stepIn ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt EmptyObject.gen ), ResponseMessage.enc_opt Dap_commands.stepIn EmptyObject.enc)
let gen_stepOutResponse = (stepOutResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make_opt ~seq ~request_seq ~success ~command:Dap_commands.stepOut ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt EmptyObject.gen ), ResponseMessage.enc_opt Dap_commands.stepOut EmptyObject.enc)
let gen_stepBackResponse = (stepBackResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make_opt ~seq ~request_seq ~success ~command:Dap_commands.stepBack ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt EmptyObject.gen ), ResponseMessage.enc_opt Dap_commands.stepBack EmptyObject.enc)
let gen_reverseContinueResponse = (reverseContinueResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make_opt ~seq ~request_seq ~success ~command:Dap_commands.reverseContinue ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt EmptyObject.gen ), ResponseMessage.enc_opt Dap_commands.reverseContinue EmptyObject.enc)
let gen_restartFrameResponse = (restartFrameResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make_opt ~seq ~request_seq ~success ~command:Dap_commands.restartFrame ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt EmptyObject.gen ), ResponseMessage.enc_opt Dap_commands.restartFrame EmptyObject.enc)
let gen_gotoResponse = (gotoResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make_opt ~seq ~request_seq ~success ~command:Dap_commands.goto ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt EmptyObject.gen ), ResponseMessage.enc_opt Dap_commands.goto EmptyObject.enc)
let gen_pauseResponse = (pauseResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make_opt ~seq ~request_seq ~success ~command:Dap_commands.pause ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt EmptyObject.gen ), ResponseMessage.enc_opt Dap_commands.pause EmptyObject.enc)
let gen_stackTraceResponse = (stackTraceResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make ~seq ~request_seq ~success ~command:Dap_commands.stackTrace ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt StackTraceResponse_body.gen ), ResponseMessage.enc Dap_commands.stackTrace StackTraceResponse_body.enc)
let gen_scopesResponse = (scopesResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make ~seq ~request_seq ~success ~command:Dap_commands.scopes ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt ScopesResponse_body.gen ), ResponseMessage.enc Dap_commands.scopes ScopesResponse_body.enc)
let gen_variablesResponse = (variablesResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make ~seq ~request_seq ~success ~command:Dap_commands.variables ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt VariablesResponse_body.gen ), ResponseMessage.enc Dap_commands.variables VariablesResponse_body.enc)
let gen_setVariableResponse = (setVariableResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make ~seq ~request_seq ~success ~command:Dap_commands.setVariable ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt SetVariableResponse_body.gen ), ResponseMessage.enc Dap_commands.setVariable SetVariableResponse_body.enc)
let gen_sourceResponse = (sourceResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make ~seq ~request_seq ~success ~command:Dap_commands.source ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt SourceResponse_body.gen ), ResponseMessage.enc Dap_commands.source SourceResponse_body.enc)
let gen_threadsResponse = (threadsResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make ~seq ~request_seq ~success ~command:Dap_commands.threads ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt ThreadsResponse_body.gen ), ResponseMessage.enc Dap_commands.threads ThreadsResponse_body.enc)
let gen_terminateThreadsResponse = (terminateThreadsResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make_opt ~seq ~request_seq ~success ~command:Dap_commands.terminateThreads ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt EmptyObject.gen ), ResponseMessage.enc_opt Dap_commands.terminateThreads EmptyObject.enc)
let gen_modulesResponse = (modulesResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make ~seq ~request_seq ~success ~command:Dap_commands.modules ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt ModulesResponse_body.gen ), ResponseMessage.enc Dap_commands.modules ModulesResponse_body.enc)
let gen_loadedSourcesResponse = (loadedSourcesResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make ~seq ~request_seq ~success ~command:Dap_commands.loadedSources ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt LoadedSourcesResponse_body.gen ), ResponseMessage.enc Dap_commands.loadedSources LoadedSourcesResponse_body.enc)
let gen_evaluateResponse = (evaluateResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make ~seq ~request_seq ~success ~command:Dap_commands.evaluate ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt EvaluateResponse_body.gen ), ResponseMessage.enc Dap_commands.evaluate EvaluateResponse_body.enc)
let gen_setExpressionResponse = (setExpressionResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make ~seq ~request_seq ~success ~command:Dap_commands.setExpression ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt SetExpressionResponse_body.gen ), ResponseMessage.enc Dap_commands.setExpression SetExpressionResponse_body.enc)
let gen_stepInTargetsResponse = (stepInTargetsResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make ~seq ~request_seq ~success ~command:Dap_commands.stepInTargets ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt StepInTargetsResponse_body.gen ), ResponseMessage.enc Dap_commands.stepInTargets StepInTargetsResponse_body.enc)
let gen_gotoTargetsResponse = (gotoTargetsResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make ~seq ~request_seq ~success ~command:Dap_commands.gotoTargets ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt GotoTargetsResponse_body.gen ), ResponseMessage.enc Dap_commands.gotoTargets GotoTargetsResponse_body.enc)
let gen_completionsResponse = (completionsResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make ~seq ~request_seq ~success ~command:Dap_commands.completions ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt CompletionsResponse_body.gen ), ResponseMessage.enc Dap_commands.completions CompletionsResponse_body.enc)
let gen_exceptionInfoResponse = (exceptionInfoResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make ~seq ~request_seq ~success ~command:Dap_commands.exceptionInfo ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt ExceptionInfoResponse_body.gen ), ResponseMessage.enc Dap_commands.exceptionInfo ExceptionInfoResponse_body.enc)
let gen_readMemoryResponse = (readMemoryResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make_opt ~seq ~request_seq ~success ~command:Dap_commands.readMemory ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt ReadMemoryResponse_body.gen ), ResponseMessage.enc_opt Dap_commands.readMemory ReadMemoryResponse_body.enc)
let gen_writeMemoryResponse = (writeMemoryResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make_opt ~seq ~request_seq ~success ~command:Dap_commands.writeMemory ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt WriteMemoryResponse_body.gen ), ResponseMessage.enc_opt Dap_commands.writeMemory WriteMemoryResponse_body.enc)
let gen_disassembleResponse = (disassembleResponse, QCheck.Gen.( map (fun (seq, request_seq, success, message, body) -> ResponseMessage.make_opt ~seq ~request_seq ~success ~command:Dap_commands.disassemble ?message ~body ()) @@ tup5 Gen.gen_int31 Gen.gen_int31 bool Gen.gen_utf8_str_opt DisassembleResponse_body.gen ), ResponseMessage.enc_opt Dap_commands.disassemble DisassembleResponse_body.enc)
 ...
 type _ t = 
       ...
       (* event GADT items *) | InitializedEvent : (Dap_events.initialized, EmptyObject.t option, Presence.opt) EventMessage.t -> (Dap_events.initialized, EmptyObject.t option, Presence.opt) EventMessage.t t
| StoppedEvent : (Dap_events.stopped, StoppedEvent_body.t, Presence.req) EventMessage.t -> (Dap_events.stopped, StoppedEvent_body.t, Presence.req) EventMessage.t t
| ContinuedEvent : (Dap_events.continued, ContinuedEvent_body.t, Presence.req) EventMessage.t -> (Dap_events.continued, ContinuedEvent_body.t, Presence.req) EventMessage.t t
| ExitedEvent : (Dap_events.exited, ExitedEvent_body.t, Presence.req) EventMessage.t -> (Dap_events.exited, ExitedEvent_body.t, Presence.req) EventMessage.t t
| TerminatedEvent : (Dap_events.terminated, TerminatedEvent_body.t option, Presence.opt) EventMessage.t -> (Dap_events.terminated, TerminatedEvent_body.t option, Presence.opt) EventMessage.t t
| ThreadEvent : (Dap_events.thread, ThreadEvent_body.t, Presence.req) EventMessage.t -> (Dap_events.thread, ThreadEvent_body.t, Presence.req) EventMessage.t t
| OutputEvent : (Dap_events.output, OutputEvent_body.t, Presence.req) EventMessage.t -> (Dap_events.output, OutputEvent_body.t, Presence.req) EventMessage.t t
| BreakpointEvent : (Dap_events.breakpoint, BreakpointEvent_body.t, Presence.req) EventMessage.t -> (Dap_events.breakpoint, BreakpointEvent_body.t, Presence.req) EventMessage.t t
| ModuleEvent : (Dap_events.module_, ModuleEvent_body.t, Presence.req) EventMessage.t -> (Dap_events.module_, ModuleEvent_body.t, Presence.req) EventMessage.t t
| LoadedSourceEvent : (Dap_events.loadedSource, LoadedSourceEvent_body.t, Presence.req) EventMessage.t -> (Dap_events.loadedSource, LoadedSourceEvent_body.t, Presence.req) EventMessage.t t
| ProcessEvent : (Dap_events.process, ProcessEvent_body.t, Presence.req) EventMessage.t -> (Dap_events.process, ProcessEvent_body.t, Presence.req) EventMessage.t t
| CapabilitiesEvent : (Dap_events.capabilities, CapabilitiesEvent_body.t, Presence.req) EventMessage.t -> (Dap_events.capabilities, CapabilitiesEvent_body.t, Presence.req) EventMessage.t t
| ProgressStartEvent : (Dap_events.progressStart, ProgressStartEvent_body.t, Presence.req) EventMessage.t -> (Dap_events.progressStart, ProgressStartEvent_body.t, Presence.req) EventMessage.t t
| ProgressUpdateEvent : (Dap_events.progressUpdate, ProgressUpdateEvent_body.t, Presence.req) EventMessage.t -> (Dap_events.progressUpdate, ProgressUpdateEvent_body.t, Presence.req) EventMessage.t t
| ProgressEndEvent : (Dap_events.progressEnd, ProgressEndEvent_body.t, Presence.req) EventMessage.t -> (Dap_events.progressEnd, ProgressEndEvent_body.t, Presence.req) EventMessage.t t
| InvalidatedEvent : (Dap_events.invalidated, InvalidatedEvent_body.t, Presence.req) EventMessage.t -> (Dap_events.invalidated, InvalidatedEvent_body.t, Presence.req) EventMessage.t t
| MemoryEvent : (Dap_events.memory, MemoryEvent_body.t, Presence.req) EventMessage.t -> (Dap_events.memory, MemoryEvent_body.t, Presence.req) EventMessage.t t


       (* event constructors *) let initializedEvent ev = InitializedEvent ev
...
let memoryEvent ev = MemoryEvent ev
 ...
 (* event qcheck *) let gen_initializedEvent = (initializedEvent, QCheck.Gen.( map (fun (seq, body) -> EventMessage.make_opt ~seq ~event:Dap_events.initialized ~body ()) @@ tup2 Gen.gen_int31 EmptyObject.gen ), EventMessage.enc_opt Dap_events.initialized EmptyObject.enc)
let gen_stoppedEvent = (stoppedEvent, QCheck.Gen.( map (fun (seq, body) -> EventMessage.make ~seq ~event:Dap_events.stopped ~body ()) @@ tup2 Gen.gen_int31 StoppedEvent_body.gen ), EventMessage.enc Dap_events.stopped StoppedEvent_body.enc)
let gen_continuedEvent = (continuedEvent, QCheck.Gen.( map (fun (seq, body) -> EventMessage.make ~seq ~event:Dap_events.continued ~body ()) @@ tup2 Gen.gen_int31 ContinuedEvent_body.gen ), EventMessage.enc Dap_events.continued ContinuedEvent_body.enc)
let gen_exitedEvent = (exitedEvent, QCheck.Gen.( map (fun (seq, body) -> EventMessage.make ~seq ~event:Dap_events.exited ~body ()) @@ tup2 Gen.gen_int31 ExitedEvent_body.gen ), EventMessage.enc Dap_events.exited ExitedEvent_body.enc)
let gen_terminatedEvent = (terminatedEvent, QCheck.Gen.( map (fun (seq, body) -> EventMessage.make_opt ~seq ~event:Dap_events.terminated ~body ()) @@ tup2 Gen.gen_int31 TerminatedEvent_body.gen ), EventMessage.enc_opt Dap_events.terminated TerminatedEvent_body.enc)
let gen_threadEvent = (threadEvent, QCheck.Gen.( map (fun (seq, body) -> EventMessage.make ~seq ~event:Dap_events.thread ~body ()) @@ tup2 Gen.gen_int31 ThreadEvent_body.gen ), EventMessage.enc Dap_events.thread ThreadEvent_body.enc)
let gen_outputEvent = (outputEvent, QCheck.Gen.( map (fun (seq, body) -> EventMessage.make ~seq ~event:Dap_events.output ~body ()) @@ tup2 Gen.gen_int31 OutputEvent_body.gen ), EventMessage.enc Dap_events.output OutputEvent_body.enc)
let gen_breakpointEvent = (breakpointEvent, QCheck.Gen.( map (fun (seq, body) -> EventMessage.make ~seq ~event:Dap_events.breakpoint ~body ()) @@ tup2 Gen.gen_int31 BreakpointEvent_body.gen ), EventMessage.enc Dap_events.breakpoint BreakpointEvent_body.enc)
let gen_moduleEvent = (moduleEvent, QCheck.Gen.( map (fun (seq, body) -> EventMessage.make ~seq ~event:Dap_events.module_ ~body ()) @@ tup2 Gen.gen_int31 ModuleEvent_body.gen ), EventMessage.enc Dap_events.module_ ModuleEvent_body.enc)
let gen_loadedSourceEvent = (loadedSourceEvent, QCheck.Gen.( map (fun (seq, body) -> EventMessage.make ~seq ~event:Dap_events.loadedSource ~body ()) @@ tup2 Gen.gen_int31 LoadedSourceEvent_body.gen ), EventMessage.enc Dap_events.loadedSource LoadedSourceEvent_body.enc)
let gen_processEvent = (processEvent, QCheck.Gen.( map (fun (seq, body) -> EventMessage.make ~seq ~event:Dap_events.process ~body ()) @@ tup2 Gen.gen_int31 ProcessEvent_body.gen ), EventMessage.enc Dap_events.process ProcessEvent_body.enc)
let gen_capabilitiesEvent = (capabilitiesEvent, QCheck.Gen.( map (fun (seq, body) -> EventMessage.make ~seq ~event:Dap_events.capabilities ~body ()) @@ tup2 Gen.gen_int31 CapabilitiesEvent_body.gen ), EventMessage.enc Dap_events.capabilities CapabilitiesEvent_body.enc)
let gen_progressStartEvent = (progressStartEvent, QCheck.Gen.( map (fun (seq, body) -> EventMessage.make ~seq ~event:Dap_events.progressStart ~body ()) @@ tup2 Gen.gen_int31 ProgressStartEvent_body.gen ), EventMessage.enc Dap_events.progressStart ProgressStartEvent_body.enc)
let gen_progressUpdateEvent = (progressUpdateEvent, QCheck.Gen.( map (fun (seq, body) -> EventMessage.make ~seq ~event:Dap_events.progressUpdate ~body ()) @@ tup2 Gen.gen_int31 ProgressUpdateEvent_body.gen ), EventMessage.enc Dap_events.progressUpdate ProgressUpdateEvent_body.enc)
let gen_progressEndEvent = (progressEndEvent, QCheck.Gen.( map (fun (seq, body) -> EventMessage.make ~seq ~event:Dap_events.progressEnd ~body ()) @@ tup2 Gen.gen_int31 ProgressEndEvent_body.gen ), EventMessage.enc Dap_events.progressEnd ProgressEndEvent_body.enc)
let gen_invalidatedEvent = (invalidatedEvent, QCheck.Gen.( map (fun (seq, body) -> EventMessage.make ~seq ~event:Dap_events.invalidated ~body ()) @@ tup2 Gen.gen_int31 InvalidatedEvent_body.gen ), EventMessage.enc Dap_events.invalidated InvalidatedEvent_body.enc)
let gen_memoryEvent = (memoryEvent, QCheck.Gen.( map (fun (seq, body) -> EventMessage.make ~seq ~event:Dap_events.memory ~body ()) @@ tup2 Gen.gen_int31 MemoryEvent_body.gen ), EventMessage.enc Dap_events.memory MemoryEvent_body.enc)
